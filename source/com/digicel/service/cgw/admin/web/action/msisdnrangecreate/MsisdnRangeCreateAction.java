...
/**
 * This class is in charge of retrieve the MsisdnRangeForm, create an MsisdnRangeBean with 
 * the data of the form and communicate with the Business Logic layer to insert a range in 
 * the B/W list. In case the business layer provides a successful operation the method 
 * forwards to the list of ranges. In case of failure the method redirects to error.
 * 
 * @author CME
 *
 */
public class MsisdnRangeCreateAction extends AbstractProtectedAction
{
   private static Logger log = LoggerFactory.getLogger(MsisdnRangeCreateAction.class);
      ...
      throws OperationNotPermittedException
   {
      ...
      if(StringUtils.isNotBlank(from) && StringUtils.isNotBlank(to))
      {
         try
         {
            ...
            MsisdnRangeManager.getInstance().insertMsisdnRange(msisdnRange);
         ...
         catch(NumberFormatException ex)
         {
            ...
            errors.add("error.parserange", new ActionMessage("error.createmsisdnrange.parserange"));
         }
         catch(Exception ex)
         {
            log.error("Error calling executeAction. Method params: " + Arrays.toString(new Object[]{mapping, form, request, response, currentUser }), ex);
      ...
      else
      {
         ...
         errors.add("error.missingrange", new ActionMessage("error.createmsisdnrange.missingrange"));
      }
