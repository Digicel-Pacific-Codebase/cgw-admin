package com.digicel.service.cgw.admin.web.action.msisdnrangedelete;
...
/**
 * This class is in charge of retrieving the id of the range from the form and 
 * communicating with the Business Logic layer to remove the range from the B/W 
 * list. In case the business layer provides a successful operation the method 
 * forwards to the list of ranges. In case of failure the method redirects to error.
 * 
 * @author CME
 *
 */
public class MsisdnRangeDeleteAction extends AbstractProtectedAction
{
   private static Logger log = LoggerFactory.getLogger(MsisdnRangeDeleteAction.class);

   /**
    * Deletes a MSISDN range retrieving the id from the request.
    */
   @Override
      ...
      throws OperationNotPermittedException
   {
      ...
      this.checkRole(request, Constants.RESOURCE_MSISDN_RANGE_DELETE);
      ActionForward result = mapping.findForward("error");
      ActionErrors errors = new ActionErrors();

      String id = request.getParameter(Constants.URL_MSISDN_RANGE_ID);
      ...
      if(StringUtils.isNotBlank(id))
      {
         try
         {
            ...
            MsisdnRangeManager.getInstance().deleteMsisdnRange(idValue);
         ...
         catch(NumberFormatException ex)
         {
            ...
            errors.add("error.parseid", new ActionMessage("error.deletemsisdnrange.parseid"));
         }
         catch(Exception ex)
         {
            log.error("Error calling executeAction. Method params: " + Arrays.toString(new Object[]{mapping, form, request, response, currentUser }), ex);
      ...
      else
      {
         ...
         errors.add("error.missingid", new ActionMessage("error.deletemsisdnrange.missingid"));
      }
